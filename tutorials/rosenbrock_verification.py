...
def main_script_grid():
    ...
    cnt_y = np.array([global_verif.outputs[i] for i in range(len(global_verif.outputs))
                      if global_verif.outputs[i] is not None])
    y_exp_min, y_exp_max = compute_bounds(container.stability_property,
                                          y_pred,
                                          is_input=False)
    print('expected',
          [(y_exp_min[i], y_pred[i], y_exp_max[i], global_verif.outputs[i]) for i in range(len(global_verif.outputs))
           if global_verif.outputs[i] is not None])
    ...
    for index_counter_example in range(cnt_x.shape[0]):
        ...
        x_min_, x_max_ = compute_bounds(container.stability_property,
                                        vals,
                                        is_input=True)
        ...
        y_min_, y_max_ = compute_bounds(container.stability_property,
                                        output_property,
                                        is_input=False)
